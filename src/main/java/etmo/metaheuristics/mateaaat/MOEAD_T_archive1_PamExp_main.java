...
public class MOEAD_T_archive1_PamExp_main {
	static int MAX_POPULATION_SIZE = 100;
	static int MAX_EVALUATION_PER_INDIVIDUAL = 1000;
	static boolean LOG_IGD = true;

	public static MtoAlgorithm algorithmGenerate(ProblemSet problemSet, int t1, int t2) throws JMException, InstantiationException, IllegalAccessException, ClassNotFoundException, NoSuchMethodException, InvocationTargetException {
		...
		algorithm.setInputParameter("t1", t1);
		algorithm.setInputParameter("t2", t2);
			...
			for (int t2 = 4; t2 <= 5; t2 ++){
				...
				for (int pCase = taskStart; pCase <= taskEnd; pCase++){
					...
					for (int t = 0; t < times; t++){
						algorithms.add(algorithmGenerate(problemSet, t1, t2));
					}
					// 并行执行times个算法
