...
public class MaTBMLx_compare1 extends MtoAlgorithm{
    ...
    public MaTBMLx_compare1(ProblemSet problemSet){
        super(problemSet);
    }
    ...
    private void transferConverge(int times) throws JMException, ClassNotFoundException {
        for (int k = 0; k < taskNum; k++){
            int sourceTask = k;
            while (sourceTask == k)
                sourceTask = PseudoRandom.randInt(0, taskNum - 1);
            transfer(k, sourceTask);
        }
