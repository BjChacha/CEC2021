...
public class MaTBMLx_PUNISH_main {
    ...
    public static void main(String[] args) throws JMException, ClassNotFoundException, NoSuchMethodException, InvocationTargetException, IllegalAccessException {
        ...
        for (int I_INIT_SCORE = 1; I_INIT_SCORE <= 3; I_INIT_SCORE++) {
            for (int I_BETTER_THRES = 1; I_BETTER_THRES <= 4; I_BETTER_THRES++) {
                int initScore = I_INIT_SCORE;
                double betterThreshold = I_BETTER_THRES * 0.05;
                ...
                for (int pCase = problemStart; pCase <= problemEnd; pCase++) {
                    ...
                    algorithm.setInputParameter("initScore", initScore);
                    algorithm.setInputParameter("betterThreshold", betterThreshold);
...
//            crossover = CrossoverFactory.getCrossoverOperator("RandomDECrossover",parameters);
                    ...
                    if (LOG_IGD) {
                        LogIGD.LogIGD("MaTBMLx(MIX_PUNISH_" + I_INIT_SCORE +"_" + I_BETTER_THRES + ")" + "_" + benchmark_name, pCase, igds);
                    }
