...
public class MaTBML_E extends MtoAlgorithm {
    ...
    public MaTBML_E(ProblemSet problemSet){
        super(problemSet);
    }
...
//            }
            ...
            SolutionSet tansferSet = new SolutionSet(checkSize);
            ...
            for (int i = 0; i < checkSize; i++) {
                ...
                if (lastBetterRate[k][groups_[k]] < 0)
                    ...
                    stopP = (i - tansferSet.size()) / checkSize;
                else
                    stopP = ((i - tansferSet.size()) - tansferSet.size()/lastBetterRate[k][groups_[k]]) / checkSize;
                // DEBUG
//                System.out.println("stop P: " + stopP);
            ...
            SolutionSet union = populations_[k].union(tansferSet);
            ...
            lastBetterRate[k][groups_[k]] = (double)transferCount / populations_[k].size();
//            System.out.println((double)evaluations_ * 100 / maxEvaluations_ + "%: 显式迁移存活率：" + ((double) transferCount / tansferSet.size()));
        }
        updateIdealPoint();
        updateNadirPoint();
    }
