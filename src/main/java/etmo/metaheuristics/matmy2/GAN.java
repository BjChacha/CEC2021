...
public class GAN {
    ...
    public void fit(double[][] input, double[][] output){
        INDArray realPops = new NDArray(output);
        // [0,1] -> [-1, 1]
        realPops.muli(2).subi(1);

        int batchSize = (int) realPops.shape()[0];

        INDArray latent = new NDArray(input);
        INDArray fakePops = new NDArray(generator.output(latent).toDoubleMatrix());

        DataSet realSet = new DataSet(realPops, Nd4j.zeros(batchSize, 1));
        DataSet fakeSet = new DataSet(fakePops, Nd4j.ones(batchSize, 1));
        ...
        INDArray adversarialLatent = new NDArray(Utils.Sample(input, batchSize));
        INDArray misleadingLabels = Nd4j.zeros(batchSize, 1);
        DataSet adversarialSet = new DataSet(adversarialLatent, misleadingLabels);
